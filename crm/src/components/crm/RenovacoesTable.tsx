...
export function RenovacoesTable({ itens, clientes, onMarcarContactado, onAbrirRenovar }: RenovacoesTableProps) {
            ...
            {itens.map((item) => {
              ...
              return (
                <tr key={apolice.id} className="border-t border-border hover:bg-ink/[0.02] transition-colors">
                  ...
                  <td className="whitespace-nowrap">{nomeCliente(apolice.cliente_id)}</td>
                    ...
                    {!renovada && (
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() => onMarcarContactado(item)}
                          disabled={estado === 'contactado'}
                        >
                          <span className="hidden sm:inline">Marcar contactado</span>
                        </Button>
                        <Button variant="primary" size="sm" icon={<RefreshCw />} aria-label="Marcar renovada" onClick={() => onAbrirRenovar(item)}>
                          <span className="hidden sm:inline">Marcar renovada</span>
                        </Button>
                      </div>
                    )}
                  </td>
